...
int solve(vector<Event> &events)
{
    ...
    int max_guard_id = -1;
    int minute_max_asleep = 0;
    int max_asleep = -1;

    for (auto guard : guard_counter)
    {
        vector<int> temp_counter = {guard.second[0]};
        for (int i = 1; i < 60; i++)
        {
            guard.second[i] += guard.second[i - 1];
            temp_counter.push_back(guard.second[i]);
        }
        int max_index = max_element(temp_counter.begin(), temp_counter.end()) - temp_counter.begin();
        if (temp_counter[max_index] > max_asleep)
        {
            max_asleep = temp_counter[max_index];
            max_guard_id = guard.first;
            minute_max_asleep = max_index;
        }
    }

    return max_guard_id * minute_max_asleep;
}
